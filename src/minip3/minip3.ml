...
In bash, before running toplevel, or executing native code, the
following may improve performance (but is not necessary):
...
using the rest of the code:
    ...
    (* #cd "/tmp/l/general/research/parsing/src/minip3";; *)
...
let run_parser3' oracle p s len = (
  let i0 = { 
    ss4=(SS(s,0,len)); 
    box4=(box s);
    lc4=empty_context; 
...
open Earley.Earley_interface (* for field selectors *)
type item = Earley.Earley_interface.nt_item
...
let earley_prods_of_parser p s len = (
  ...
  let r = Earley.Earley_interface.earley_full s in
  let prods = match r with `Loop2(`Done(_),`Prod(prods)) -> prods in
  let elts = Earley.Earley_interface.Set_nt_item_int.elements prods in (* FIXME performance may want to avoid conversion to list; if so, we probably need a more efficient version of fold than in the standard lib? probably not *)
  elts)
(*
  let map = rev_map in
  map (fun (itm,k) -> (item_of_p3 itm,k)) elts) (* FIXME this map is inefficient! *) *)
let (_:'a parser3' -> string -> int -> production list) = earley_prods_of_parser

let oracle_of_prods ps = (fun (sym1,sym2) -> fun (i',j') ->
...
module Map_sym_sym_int_int = Earley.Earley_private_types.MyMap(
  struct
...
let _ = run_parser3' oracle parse_E "1111111" 7
(* - : int list = [7] *)
...
let ps = earley_prods_of_parser parse_E "1111111" 7
