module Earley_int_interface = struct
  ...
  type 'a ty_setup = <
    g7      : grammar;
    sym7    : nonterm;
    p_of_tm7: (term -> ('a substring -> int list)) ; (* make sure this is efficient! *)
  ...
  let earley_full (setup0:'a ty_setup) = (
    let (g,nt,p_of_tm,s,l) = (setup0#g7,setup0#sym7,setup0#p_of_tm7,setup0#string7,setup0#length7) in
    let setup0 =
      object
        method std_g=g;
        method std_sym=nt;
        method std_p_of_tm=p_of_tm;
        method std_string=s;
        method std_length=l
      end
    in
    E3_std.earley_full setup0)
  let (_:'a ty_setup -> ty_result) = earley_full
