...
module Earley_private_types = struct
  ...
  module Map_sym_sym_int_int = (
    struct
      ...
      type ty_map = Map_int_2.ty_map

      (* m is of type Map_int_2.ty_map; k is a set of int *)
      let add k v mi2 = (
        let (s1,s2,x,y) = k in
        let mi1 = Map_int_2.find2 x mi2 in
        let mss = Map_int_1.find2 y mi1 in
(*        let ks = Map_sym_sym.find2 (s1,s2) mss in
        let ks' = Set_int.add v ks in *)
        let mss = Map_sym_sym.add (s1,s2) v mss in
        let mi1 = Map_int_1.add y mss mi1 in
        let mi2 = Map_int_2.add x mi1 mi2 in
        mi2)
...
(*  let update_tmoracle m (tm,i,j) = m *)
  ...
  let earley_initial_state ctxt0 = ({
      todo_done5=Set_todo_done_item.empty;
      todo5=(
